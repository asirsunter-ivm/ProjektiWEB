...
if (!preg_match('/^PW[A-Z0-9]+$/', $tracking)) {
    echo json_encode(['success' => false, 'message' => 'Format i pavlefshem i kodit tracking.']);
    exit;
}
...
try {
    ...
    if (!$package) {
        echo json_encode(['success' => false, 'message' => 'Asnje pako nuk u gjet me kete kod.']);
        exit;
    }

    $stmt = $pdo->prepare('SELECT status, location, note, created_at
    ...
    echo json_encode(['success' => false, 'message' => 'Gabim ne server. Provoni perseri.']);
}